"""
Unit tests for BiLSTM deep learning model architecture, sequence dimensions, and interface.
"""
...
try:
    ...
    from src.models.bilstm import BiLSTMModel, TF_AVAILABLE
...
def test_bilstm_initialization():
    """Test BiLSTM hyperparameter initialization."""
    if not DL_AVAILABLE:
    ...
    assert model_wrapper.lstm_units == (64, 32)
...
def test_bilstm_architecture_build():
    """Test model construction and layer shapes."""
    if not DL_AVAILABLE:
        pytest.skip("TensorFlow / NumPy C-extensions not accessible in current environment")

    model_wrapper = BiLSTMModel(num_features=46, num_classes=8)
...
def test_bilstm_fit_predict(tmp_path):
    ...
    model_wrapper = BiLSTMModel(num_features=46, num_classes=4)
    ...
    loaded_wrapper = BiLSTMModel(num_features=46, num_classes=4)
