"""
Training and Evaluation Script for BiLSTM Deep Learning Model.

Usage:
    python scripts/train_bilstm.py --data-dir data/processed --epochs 30 --batch-size 128
"""
...
def train_and_evaluate_bilstm(
    ...
    """
    Train and evaluate the BiLSTM deep learning model.
    """
    ...
    print("=" * 75)
    print("      ML-IDS: BiLSTM DEEP LEARNING MODEL TRAINING & EVALUATION")
    print("=" * 75)
    ...
    print(f"      Reshaping input for BiLSTM: (samples, {num_features}, 1)")

    # 2. Build BiLSTM Model
    print("[2/7] Constructing BiLSTM architecture...")
    ...
    # 4. Train Model
    print(f"[3/7] Training BiLSTM (epochs={epochs}, batch_size={batch_size}, lr={learning_rate})...")
    ...
    hist_df.to_csv(metric_path / "bilstm_history.csv", index_label="epoch")
    ...
    try:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(graph_path / "bilstm_training_loss.png", dpi=300)
        plt.close()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(graph_path / "bilstm_training_accuracy.png", dpi=300)
        plt.close()
    ...
    y_val_pred = bilstm_wrapper.predict(X_val)
    t_val_end = time.time()
    ...
    with open(metric_path / "bilstm_validation_metrics.json", "w", encoding="utf-8") as f:
        json.dump(val_metrics, f, indent=4)

    val_report_dict = classification_report(y_val, y_val_pred, target_names=target_names, output_dict=True, zero_division=0)
    pd.DataFrame(val_report_dict).transpose().to_csv(metric_path / "bilstm_validation_report.csv")
    ...
    y_test_pred = bilstm_wrapper.predict(X_test)
    ...
    with open(metric_path / "bilstm_test_metrics.json", "w", encoding="utf-8") as f:
    ...
    test_report_df.to_csv(metric_path / "bilstm_test_report.csv")
    test_report_df.to_csv(metric_path / "bilstm_per_class.csv")
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(conf_path / "bilstm_validation.png", dpi=300)
        plt.close()
        ...
        plt.title("BiLSTM Deep Learning — Test Confusion Matrix")
        plt.xlabel("Predicted Class")
        plt.ylabel("True Class")
        plt.xticks(rotation=45, ha="right")
        plt.tight_layout()
        plt.savefig(conf_path / "bilstm_test.png", dpi=300)
        plt.close()
    ...
    misclass_df.to_csv(metric_path / "bilstm_misclassifications.csv", index=False)
    ...
    with open(metric_path / "bilstm_runtime.json", "w", encoding="utf-8") as f:
        json.dump(runtime_records, f, indent=4)

    # Update Model Comparison Table (Preserving Random Forest, XGBoost, 1D-CNN)
    ...
    print("=" * 75)
    print("STATUS: BiLSTM Training & Evaluation Completed.")
    return test_metrics


def main():
    parser = argparse.ArgumentParser(description="Train and Evaluate BiLSTM Model")
    parser.add_argument("--data-dir", type=str, default="data/processed", help="Path to processed dataset directory")
    parser.add_argument("--models-dir", type=str, default="models/bilstm", help="Model saving directory")
    ...
    try:
        ...
        print(f"\n[ERROR] BiLSTM training halted: {str(e)}", file=sys.stderr)
        sys.exit(1)
