"""
Training and Evaluation Script for Random Forest Baseline Model.
...
def train_and_evaluate_rf(
    ...
    print("=" * 75)
    print("      ML-IDS: RANDOM FOREST BASELINE MODEL TRAINING & EVALUATION")
    print("=" * 75)
    ...
    # Load feature names if available in preprocessing scaler
    feature_names = []
    ...
    print(f"      Train Samples: {len(X_train):,} | Features: {X_train.shape[1]} | Classes: {len(target_names)}")
    print(f"      Val Samples:   {len(X_val):,}")
    print(f"      Test Samples:  {len(X_test):,}")

    # 2. Train Model
    print(f"[2/7] Training Random Forest (n_estimators={n_estimators}, class_weight={class_weight})...")
    ...
    with open(metric_path / "random_forest_validation_metrics.json", "w", encoding="utf-8") as f:
        json.dump(val_metrics, f, indent=4)

    val_report_dict = classification_report(y_val, y_val_pred, target_names=target_names, output_dict=True, zero_division=0)
    pd.DataFrame(val_report_dict).transpose().to_csv(metric_path / "random_forest_validation_report.csv")
    ...
    with open(metric_path / "random_forest_test_metrics.json", "w", encoding="utf-8") as f:
    ...
    test_report_df.to_csv(metric_path / "random_forest_test_report.csv")
    test_report_df.to_csv(metric_path / "random_forest_per_class.csv")
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(conf_path / "random_forest_validation.png", dpi=300)
        plt.close()
        ...
        plt.title("Random Forest Baseline — Test Confusion Matrix")
        plt.xlabel("Predicted Class")
        plt.ylabel("True Class")
        plt.xticks(rotation=45, ha="right")
        plt.tight_layout()
        plt.savefig(conf_path / "random_forest_test.png", dpi=300)
        plt.close()
    ...
    print("[6/7] Extracting model-derived feature importances...")
    feat_imp = rf_model.get_feature_importances(feature_names=feature_names)
    feat_imp_df = pd.DataFrame(list(feat_imp.items()), columns=["Feature", "Importance"]).sort_values(by="Importance", ascending=False)
    feat_imp_df.to_csv(metric_path / "random_forest_feature_importance.csv", index=False)

    try:
        import matplotlib.pyplot as plt
        top20 = feat_imp_df.head(20)
        plt.figure(figsize=(10, 6))
        plt.barh(top20["Feature"][::-1], top20["Importance"][::-1], color="#2b5c8f")
        plt.title("Random Forest — Top 20 Feature Importances (Gini)")
        plt.xlabel("Importance")
        plt.tight_layout()
        plt.savefig(graph_path / "random_forest_feature_importance.png", dpi=300)
        plt.close()
        print(f"      Feature importance plot saved to {graph_path / 'random_forest_feature_importance.png'}")
    except Exception as e:
    ...
    misclass_df.to_csv(metric_path / "random_forest_misclassifications.csv", index=False)
    ...
    with open(metric_path / "random_forest_runtime.json", "w", encoding="utf-8") as f:
    ...
    rf_model.save(model_path / "random_forest.pkl")
    metadata = {
    ...
    with open(model_path / "random_forest_metadata.json", "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=4)

    print(f"      Model saved to: {model_path / 'random_forest.pkl'}")
    print(f"      Metadata saved to: {model_path / 'random_forest_metadata.json'}")
    print("=" * 75)
    print("STATUS: Random Forest Baseline Training & Evaluation Completed.")
    return test_metrics
...
def main():
    ...
    parser.add_argument("--class-weight", type=str, default=None, help="Class weighting ('balanced' or None)")
    ...
    try:
        ...
        print(f"\n[ERROR] Random Forest training halted: {str(e)}", file=sys.stderr)
        sys.exit(1)
